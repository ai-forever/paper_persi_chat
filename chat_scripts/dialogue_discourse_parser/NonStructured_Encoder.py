...
class NonStructured_Encoder():
    def __init__(self, sess, FLAGS, embed, num_units=None, scope="Sentence_Encoder"):
        ...
        for var in tf.trainable_variables():
            ...
            if var.name.find(os.path.join(tensorflow.get_current_name_scope(), scope)) == 0:
    ...
    def _build_embedding(self, embed):
        self.symbol2index = tf.lookup.experimental.MutableHashTable( #tf.contrib.lookup.MutableHashTable(
            key_dtype=tf.string,
            ...
            name="in_table",
            checkpoint=True)
        self.index2symbol = tf.lookup.experimental.MutableHashTable( #tf.contrib.lookup.MutableHashTable(
            key_dtype=tf.int64,
    ...
    def _build_encoder(self, inputs, length, input_size, use_biencoder, scope, reuse=None):
        with tf.variable_scope(scope, reuse=reuse):
            if use_biencoder:
                cell_fw, cell_bw = self._build_biencoder_cell(int(input_size))
    ...
    def _build_cell(self, num_units, input_size):
        ...
        for i in range(self.num_layers):
            ...
            dropout = DropoutWrapper(
                ...
                tf.nn.rnn_cell.GRUCell(num_units), self.keep_prob,
