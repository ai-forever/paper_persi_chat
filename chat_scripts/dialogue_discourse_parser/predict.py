...
tf.flags.DEFINE_string('train_file', 'data/STAC/train.json', 'test file path')
tf.flags.DEFINE_string('test_file', 'data/STAC/test.json', 'test file path')
...
map_relations = {}
data_train = load_data(FLAGS.train_file, map_relations)
data_test = load_data(FLAGS.test_file, map_relations)
...
with sess.as_default():
    ...
    if FLAGS.train:
        ...
        with open('predictions/' + FLAGS.test_file.split('/')[-1].replace('.json', '.pkl'), 'wb') as f:
